        ...
        try {
            ...
            entradas.forEach((item, index) => {
                ...
                if (item.rostro_detectado === false) {
                    badgeHtml = `<span class="inline-flex items-center justify-center px-sm py-xs rounded bg-surface-container-highest text-text-medium font-caption text-caption uppercase border border-outline-variant/30 gap-xs shadow-sm">
                        <span class="material-symbols-outlined text-[14px]">face_retouching_off</span> Sin Rostro
                    </span>`;
                } else if (isMatch) {
                    badgeHtml = `<span class="inline-flex items-center justify-center px-sm py-xs rounded bg-status-success/10 text-status-success font-caption text-caption uppercase border border-status-success/20 gap-xs">
                        <span class="material-symbols-outlined text-[14px]">check_circle</span> Match
                    </span>`;
                } else {
                    badgeHtml = `<span class="inline-flex items-center justify-center px-sm py-xs rounded bg-status-error/10 text-status-error font-caption text-caption uppercase border border-status-error/20 gap-xs shadow-[0_0_10px_rgba(255,59,59,0.2)]">
                ...
                tr.innerHTML = `
                    <td class="p-md text-text-medium font-mono text-sm">${item.timestamp ? new Date(item.timestamp).toLocaleTimeString() : new Date().toLocaleTimeString()}</td>
                    <td class="p-md text-status-cyan font-mono text-sm">${item.archivo || item.id || `Eval #${index + 1}`}</td>
                    <td class="p-md font-mono text-sm">${confidence}</td>
                    <td class="p-md font-mono text-sm ${isMatch ? '' : 'text-status-error font-semibold'}">${distance}</td>
                    <td class="p-md text-right">${badgeHtml}</td>
                `;
